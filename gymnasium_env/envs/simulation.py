...
from gymnasium_env.envs.ground import GroundContact
...
class Quadrotor:
    def __init__(self, mass=1.0, init_position=None, init_orientation=None, dt=0.01):
        ...
        # initial state
        self.state = State(
            position=np.zeros(3) if init_position is None else init_position,
            velocity=np.zeros(3),
            orientation=np.array([0,0,0,1]) if init_orientation is None else init_orientation, 
